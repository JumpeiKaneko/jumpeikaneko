...
「東京都盲人福祉協会での鑑賞会」「気配に触れる／ミキキの交差点」「線を聴く」の3つは、
いずれも「絵画を聴く」から派生した実践だが、開催場所・形式が異なるため独立ページにした。
「気配に触れる」と「ミキキの交差点」は同じ日・同じ会場（100BANCH）での発表のため
1ページにまとめている。各ページの冒頭リンクと、絵画を聴くページの「関連展示」セクション
から相互にたどれる。
「VR学会論文」は独立ページを作らず、展示・受賞歴のリストにのみ関連情報として掲載している。

※ `work-detail-mikiki.html` は使われていない旧ファイルです。削除してよければお知らせください。
